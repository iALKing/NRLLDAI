...
def classifier():
    ...
    while counter < 10:
        allNodeFeatureNum = []
        allNodeFeatureNumName = r'../Data/LMDNFeature/FinalFeature.csv'
    ...
    StorFile(MyNew, './Result/_%s十折结果.csv'%(classifierName))
    ...
    # 保存图片
    plt.savefig('./Result/_%s10FoldAUC.svg'%(classifierName))
    plt.savefig('./Result/_%s10FoldAUC.tif'%(classifierName))
    plt.show()
