...
def classifier(method):
    ...
    ReadMyCsv2(positiveSample, r'PositiveSample.csv')
    negativeSample = []  # 已打乱顺序的正样本
    ReadMyCsv2(negativeSample, r'NeagtiveSample.csv')
    newRandomList = []
    ReadMyCsv2(newRandomList, r'RandomListGroup.csv')
    ...
    while counter < 10:
        allNodeFeatureNum = []
        allNodeFeatureNumName = r'../Data/SamLDFeature/%sFeature.csv'%method
        ...
        while counter1 < len(positiveSample):
            FeaturePair = []
            FeaturePair.extend(allNodeFeatureNum[positiveSample[counter1][0]-1])
            FeaturePair.extend(allNodeFeatureNum[positiveSample[counter1][1]-1])
        ...
        Name = './Result/' +'%s_%s_第'%(method, classifierName) + str(i+1) + '次Score.csv'
        StorFile(RealAndPrediction, Name)
        # 保存Real,Prediction.csv
        RealAndPredictionProb = myRealAndPredictionProb(TestLabel, y_score1)
        NameProb = './Result/' +'%s_%s_第'%(method, classifierName) + str(i+1) + '次Prob.csv'
    ...
    StorFile(MyNew, './Result/%s_%s十折结果.csv'%(method, classifierName))
    ...
    # 保存图片
    plt.savefig('./Result/%s_%s10FoldAUC.svg'%(method,classifierName))
    plt.savefig('./Result/%s_%s10FoldAUC.tif'%(method,classifierName))
    plt.show()


if __name__ == '__main__':
    methodList = ["LLE", "DeepWalk", "GRAREP", "Node2vec", "Hope", "Line", "SDNE"]    #for method in methodList:
    classifier(methodList[6])
