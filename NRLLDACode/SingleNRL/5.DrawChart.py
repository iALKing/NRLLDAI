...
def getPath(method, times):
        filePath = "./Result/%s_NB_第%d次Prob.csv" % (method, times)
        return filePath
...
def drawPR(methodList, colorList, styleList):
    for i in range(len(methodList)+1):
        if i == len(methodList):
            ...
            plt.plot(pre, rec, color=colorList[i], linestyle=styleList[i],
             label='%s (AUPR = %0.4f)' % (methodList[i], AP),
             lw=2, alpha=1)


def drawROC(methodList, colorList, styleList):
    for i in range(len(methodList)+1):
        if i == len(methodList):
            plt.xlim([0, 1])
            plt.ylim([0, 1])
            ...
            AUC = auc(fpr, tpr)
            plt.plot(fpr, tpr, color=colorList[i], linestyle=styleList[i],
...
if __name__=="__main__":
    # 方法名：DeepWalk、GF、GRAREP、Hope、LAP、Line、LLE、Node2vec【Node2vec】、SDNE
    methodList = ["LLE", "DeepWalk", "GRAREP", "Node2vec", "Hope", "Line", "SDNE"]
    colorList = ["black", "black", "black", "black", "black", "black", "black"]
    styleList = ['-', ':', '--', '-.', (0, (5, 10)), (0, (3, 5, 1, 5)), (0, (3, 1, 1, 1, 1, 1))]
    # colorList = ["red",  "yellow", "green", "blue",  "deeppink", "sienna", "darkviolet"]
    drawPR(methodList, colorList, styleList)
    drawROC(methodList, colorList, styleList)
